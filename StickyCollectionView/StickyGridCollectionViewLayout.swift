...
class StickyGridCollectionViewLayout: UICollectionViewFlowLayout {
    ...
    override func prepare() {
        let lastItemFrameX = setupAttributes()
        updateStickyItemsPositions()
        
        let lastItemFrameY = allAttributes.last?.last?.frame ?? .zero
        contentSize = CGSize(width: lastItemFrameX.maxX, height: lastItemFrameY.maxY)
    }
    ...
    // MARK: - Helpers
    private func setupAttributes() -> CGRect {
        allAttributes = []
        
        var xOffset: CGFloat = 0
        var yOffset: CGFloat = 0
        var furtherstColumn: CGRect!
        ...
        for row in 0..<rowsCount {
            ...
            for col in 0..<columnsCount(in: row) {
                ...
                if furtherstColumn == nil || furtherstColumn.maxX < attributes.frame.maxX {
                    furtherstColumn = attributes.frame
                }
            }
